...
class Main(object):
    ...
    def __init__(self, configDirName, configName, logFileName):
        ...
        self.__CONFIG_DIR = pkg_resources.resource_filename('display', 'etc')
        self.__USER_CONFIG_DIR = expanduser('~/.'+configDirName)
        self.__configName = configName
        self.__logFileName = logFileName
        self._checkUserConfigFiles()
        self.version = pkg_resources.require("displayCalendar")[0].version
    ...
    def _checkUserConfigFiles(self):
        ...
        if not os.path.exists(self.__pathjoin(self.__USER_CONFIG_DIR, 'logging.conf')):
            ...
            origLoggingConfig = os.path.join(self.__CONFIG_DIR, 'logging.conf')
            shutil.copy(origLoggingConfig, self.__pathjoin(self.__USER_CONFIG_DIR, 'logging.conf'))
        if not os.path.exists(self.__pathjoin(self.__USER_CONFIG_DIR, self.__configName)):
            ...
            origApplConfig = os.path.join(self.__CONFIG_DIR, self.__configName)
            shutil.copy(origApplConfig, self.__pathjoin(self.__USER_CONFIG_DIR, self.__configName))
            printWarningAndAbort = True
